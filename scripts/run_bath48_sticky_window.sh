...
CHK_BASE="outputs/chk/bath48_sticky"
STAGE1_BASE="outputs/bath48_sticky_stage1"
STAGE2_BASE="outputs/bath48_sticky"
...
echo "[sticky] stage 1: 0 -> 10.5"
"${MPIEXEC}" -np "${NP}" "${EXE}" -i "${INPUT}" "${SOLVER_OVR}" "${QUIET_OVR}" \
  Executioner/end_time=10.5 \
...
if [[ -z "${RECOVER_PREFIX}" ]]; then
  echo "[sticky] ERROR: no checkpoint found under ${CHK_BASE}_cp" >&2
  exit 1
fi

echo "[sticky] stage 2: recover -> 13.0 from ${RECOVER_PREFIX}"
"${MPIEXEC}" -np "${NP}" "${EXE}" --recover="${RECOVER_PREFIX}" -i "${INPUT}" "${SOLVER_OVR}" "${QUIET_OVR}" \
  Executioner/end_time=13.0 \
  Outputs/exodus=false \
