# yeastpuck3.i
# 3D yeast-in-gel puck with CellGel + SolidMechanics (new system) + nutrient gate
# Time units: HOURS
# Geometry: r = 1 (→ 2 mm), h = 1 (→ 1 mm). Experiment duration ≈ 48 h.
# Parameter choices are tuned to:
# - 10% acrylamide / 0.1% bis PAAm hydrogel with embedded S. cerevisiae
# - ~10^9 cells/mL (phi_cell_0 ~ 0.1) like Ware et al.
# - Glucose D in PAAm ≈ (0.3–0.8) * D_water, and realistic uptake rates.
# - Growth stratification via nutrient + pressure gating.
...
[Mesh]
  parallel_type = distributed  # required for SolutionUserObject reading (replicated mesh)
  [rect]
...
[Variables]
  # SolidMechanics will create ux, uy, uz
  [n]
    family = LAGRANGE
    order  = FIRST
  []
...
[Executioner]
  ...
  # Time units are hours. We want 48 h total experiment.
  dtmin       = 1e-4
  dtmax       = 0.1          # max step = 0.1 h = 6 min
  end_time    = 34.0
  ...
  [TimeStepper]
    ...
    dt                 = 1e-2    # initial dt = 0.01 h = 0.6 min
    growth_factor      = 1.3
    cutback_factor     = 0.7
