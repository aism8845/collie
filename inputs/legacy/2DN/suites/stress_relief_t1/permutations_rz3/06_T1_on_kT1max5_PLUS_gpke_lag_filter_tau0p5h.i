...
[AuxKernels]
  ...
  [gp_ke_used_aux]
    type     = MaterialRealAux
    variable = gp_ke_used_out
    property = gp_ke_used_out
  []
  [gp_ke_lag_alpha_aux]
    type     = MaterialRealAux
    variable = gp_ke_lag_alpha_out
    property = gp_ke_lag_alpha_out
  []
  [gate_tot_aux]
    type     = MaterialRealAux
    variable = gate_tot
...
[Outputs]
  exodus = true
  perf_graph = true
  file_base = outputs/suites/stress_relief_t1/permutations_rz3/06_T1_on_kT1max5_PLUS_gpke_lag_filter_tau0p5h/06_T1_on_kT1max5_PLUS_gpke_lag_filter_tau0p5h

  [mesh_watch]
    type = CSV
    file_base = outputs/suites/stress_relief_t1/permutations_rz3/06_T1_on_kT1max5_PLUS_gpke_lag_filter_tau0p5h/mesh_watch
    execute_on = 'TIMESTEP_END'
  ...
  [solver_watch]
    type = CSV
    file_base = outputs/suites/stress_relief_t1/permutations_rz3/06_T1_on_kT1max5_PLUS_gpke_lag_filter_tau0p5h/solver_watch
    execute_on = 'TIMESTEP_END'
    show = 'dt nonlinear_its linear_its J_min_1 metric2_min_1 mesh_distortion_warning min_elem_quality max_elem_quality min_volume_ratio max_volume_ratio min_pressure max_pressure avg_J vol_change_pct avg_n n_min n_max n_span n_min_elem n_max_elem avg_phi_cell min_phi_cell max_phi_cell avg_ke_total avg_ke_swelling avg_ke_div min_ke_total max_ke_total min_chi max_chi avg_chi avg_kT1 avg_Dphys min_Dphys max_Dphys avg_Dref min_Dref D_ref_phys_ratio avg_gate_tot min_gate_tot max_gate_tot avg_gp avg_gp_raw avg_gp_ke_used avg_press avg_pressure avg_fa avg_ke avg_kh avg_eta avg_gamma_n_local avg_n_source_ref n_corner n_bulk ke_total_corner ke_total_bulk phi_corner phi_bulk press_corner press_bulk gp_ke_used_corner gp_ke_used_bulk chi_corner chi_bulk'
  []
[]
