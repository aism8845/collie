...
[Functions]
  [n_refresh_24h]
    type = ParsedFunction
    expression = '0.2 + (1.0 - 0.2) * exp(-(t - 24.0*floor(t/24.0))/6.0)'
  []
[]
...
[Outputs]
  exodus = true
  perf_graph = true
  file_base = outputs/suites/jacobian_solver/permutations_rz3/09_periodic_refresh_boundary/09_periodic_refresh_boundary

  [mesh_watch]
    type = CSV
    file_base = outputs/suites/jacobian_solver/permutations_rz3/09_periodic_refresh_boundary/mesh_watch
    execute_on = 'TIMESTEP_END'
  ...
  [solver_watch]
    type = CSV
    file_base = outputs/suites/jacobian_solver/permutations_rz3/09_periodic_refresh_boundary/solver_watch
    execute_on = 'TIMESTEP_END'
